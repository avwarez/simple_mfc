...
// ---------------------------------------------------------------------
class CAsyncSocketReactor
{
public:
    static CAsyncSocketReactor& Instance()
    {
        static CAsyncSocketReactor inst;
        return inst;
    }

    void Add(CAsyncSocket* s)
    {
    ...
    void Remove(CAsyncSocket* s)
    {
        std::lock_guard<std::recursive_mutex> lk(m_mtx);
        m_sockets.erase(s);
    }

    CAsyncSocket* Find(SOCKET h)
    {
        std::lock_guard<std::recursive_mutex> lk(m_mtx);
        for (CAsyncSocket* s : m_sockets)
            if (s->m_hSocket == h)
                return s;
        return nullptr;
    }
...
private:
    CAsyncSocketReactor() = default;

    bool Alive(CAsyncSocket* s) { return m_sockets.count(s) != 0; }
    ...
    void Run()
    {
        while (m_running)
        {
            ...
            std::set<CAsyncSocket*> snapshot = m_sockets;
            for (CAsyncSocket* s : snapshot)
            {
    ...
    void Dispatch(CAsyncSocket* s, bool r, bool w, bool e)
    {
        const long ev = s->m_lEvent;
    ...
    std::set<CAsyncSocket*> m_sockets;
    std::thread            m_thread;
...
void FormatV4(const sockaddr_in& sa, CString& addr, UINT& port)
{
    char ip[INET_ADDRSTRLEN] = {0};
    ::inet_ntop(AF_INET, &sa.sin_addr, ip, sizeof(ip));
    addr = CString(mfc_detail::Widen(ip, std::strlen(ip)).c_str());
...
// CAsyncSocket
// ---------------------------------------------------------------------
CAsyncSocket::CAsyncSocket() noexcept
...
SOCKET CAsyncSocket::Detach()
{
    ...
    if (m_hSocket != INVALID_SOCKET)
        CAsyncSocketReactor::Instance().Remove(this);
...
BOOL CAsyncSocket::Accept(CAsyncSocket& rConnectedSocket, SOCKADDR* lpSockAddr,
                          int* lpSockAddrLen)
{
...
BOOL CAsyncSocket::SetSockOpt(int nOptionName, const void* lpOptionValue,
...
BOOL CAsyncSocket::GetSockOpt(int nOptionName, void* lpOptionValue,
...
BOOL CAsyncSocket::AsyncSelect(long lEvent)
{
    ...
    if (lEvent != 0)
        CAsyncSocketReactor::Instance().Add(this);
    else
        CAsyncSocketReactor::Instance().Remove(this);
    return TRUE;
}
...
CAsyncSocket* CAsyncSocket::FromHandle(SOCKET hSocket)
{
    if (hSocket == INVALID_SOCKET)
        return nullptr;
    return CAsyncSocketReactor::Instance().Find(hSocket);
}
