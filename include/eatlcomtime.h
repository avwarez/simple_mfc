...
#pragma once
#include "eatltime.h" // as real atlcomtime.h does; __time64_t comes from there
...
{
public:
    ECOleDateTime() noexcept;
    ECOleDateTime(const ECOleDateTime& dateSrc) noexcept;
    ECOleDateTime(DATE dtSrc) noexcept;
    ECOleDateTime(__time64_t timeSrc) noexcept;
    ECOleDateTime(const SYSTEMTIME& systimeSrc) noexcept;
    ECOleDateTime(const FILETIME& filetimeSrc) noexcept;
    ECOleDateTime(int nYear, int nMonth, int nDay, int nHour, int nMin, int nSec) noexcept;
